...
class LlamaContext {
  ...
  parallel = {
    ...
    completion: async (params, onToken) => {
      ...
      const {
        requestId
      } = await _NativeRNLlama.default.queueCompletion(this.id, nativeParams);
      ...
      const stop = async () => {
        await _NativeRNLlama.default.cancelRequest(this.id, requestId);
        // Clean up listeners
        tokenListener?.remove();
        completeListener?.remove();
      };
    ...
    embedding: async (text, params) => {
      ...
      const {
        requestId
      } = await _NativeRNLlama.default.queueEmbedding(this.id, text, params || {});
    ...
    rerank: async (query, documents, params) => {
      ...
      const {
        requestId
      } = await _NativeRNLlama.default.queueRerank(this.id, query, documents, params || {});
     ...
     */
    enable: config => _NativeRNLlama.default.enableParallelMode(this.id, {
      enabled: true,
      ...config
    }),
    /**
     ...
     * @returns Promise resolving to true if successful
     */
    disable: () => _NativeRNLlama.default.enableParallelMode(this.id, {
      enabled: false
    }),
    /**
     ...
     * @returns Promise resolving to true if successful
     */
    configure: config => _NativeRNLlama.default.enableParallelMode(this.id, {
      enabled: true,
      ...config
    })
  };
  ...
  tokenize(text, {
    ...
    return _NativeRNLlama.default.tokenize(this.id, text, mediaPaths);
  }
  detokenize(tokens) {
  ...
  async initVocoder({
    ...
    if (path.startsWith('file://')) path = path.slice(7);
    return await _NativeRNLlama.default.initVocoder(this.id, {
      path,
      n_batch: nBatch
    });
  }
...
async function getBackendDevicesInfo() {
  const jsonString = await _NativeRNLlama.default.getBackendDevicesInfo();
